	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4 && xhr.status == 200) {
			...
			if (jsonString !=-3 ) {
				...
				var container = document.getElementById("container_panier");
				console.log(container);

				if (obj == "commande valide") {
					container.innerHTML = "Votre commande a été validé";
				}
			}
...
function changeQte(event) {
	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4 && xhr.status == 200) {
			...
			var obj = JSON.parse(jsonString);
			if (obj !== undefined) {
				var tdQte = trLigneProduit.getElementsByClassName("soust");
				console.log(tdQte);
				tdQte[0].innerHTML = obj.produitJsonType.prix
						* obj.produitJsonType.quantite;
				...
				sousRecaps[0].innerHTML = "SOUS-TOTAL: "
						+ (valeurFloat).toFixed(2);
				sousRecaps[1].innerHTML = "SOUS-TOTAL: "
						+ (valeurFloat * 0.1).toFixed(2);
				sousRecaps[2].innerHTML = "SOUS-TOTAL: "
						+ (valeurFloat * 0.05).toFixed(2);
				sousRecaps[3].innerHTML = "SOUS-TOTAL: "
						+ (valeurFloat * 1.15).toFixed(2);
