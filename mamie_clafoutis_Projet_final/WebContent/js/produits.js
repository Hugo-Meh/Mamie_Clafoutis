			...
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				for(var produit of objectProduit){
					...
					if(produit.imageList!==undefined){
						if(produit.imageList[0].src_img_norm!==undefined){
						html+="<img src=images/photo/"+produit.imageList[0].src_img_norm+" class='img_prod2'/>";
						}else if(produit.imageList[0].formatWeb!==undefined){
							html+="<img src="+produit.imageList[0].formatWeb+" class='img_prod2'/>";
						}
					}
					else{
					...
					html+="<div class='div-qty-produit'>"+
					"<label>Quantitée</label>"+
					"<input class='input-form valid-caract input-qty-panier' type='number' name='qtyPanier'>"+
					"</div>";
					}
					html+="<div>";
					if(produit.quantite!=-1){
					html+="<button class='bouton btnPanier btn_ajout_prod' id='"+produit.id+"'>Ajouter au panier</button>";
					}
					html+="<button class='bouton btn_desc_prod detail-prod-class' value='"+produit.id+"'>Description</button>"+
					"</div>" +
					"</div>";
