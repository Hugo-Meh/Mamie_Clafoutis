...
public class SessionManager {
	...
	public static ParametresProduit modifierPanier(ArrayList<Produit> listProduit, Produit produit) {
		...
		if (index != -1) {
			param= new ParametresProduit();
			listProduit.get(index).setQuantite(produit.getQuantite());
			Produit produitToSend= new Produit();
			produitToSend=listProduit.get(index);
			produitToSend.setIngredientList(null);
			produitToSend.setPatte(null);
			param.setProduitJsonType(produitToSend);
			String prix = String.valueOf(prixTotal(listProduit));
			Parametre p = new Parametre("PTHT", prix);
			param.setParametreList(p);
		}
		return param;
	}
}
